...
def eligible_clubs(club_name, group_df):
    ...
    if group_subset.empty:
        raise ValueError("No elgible clubs - either no more winners or runners up")
    else:
        group_subset = group_subset.loc[(group_df['country'] != club_info.country.item())]
        if group_subset.empty:
            raise ValueError("No elgible clubs - all remaining clubs from same country")
        else: 
            group_subset = group_subset.loc[(group_df['group'] != club_info.group.item())]
            if group_subset.empty:
                raise ValueError("No elgible clubs - all remaining clubs in same group")
            else:
...
def draw_clubs_country(last16_df):
    ...
    priority_clubs = priority_clubs[priority_clubs.country.str.contains(priority_countries_pattern)]
    ...
    while not runners.empty:
        if (unchosen_clubs.shape[0] == 4) & (
            ...
            last4_group_count = pd.DataFrame(unchosen_clubs.group.value_counts().reset_index())
            last4_group_count.columns = ['group', 'group_count']
            
            last4_country_count = pd.DataFrame(unchosen_clubs.country.value_counts().reset_index())
            last4_country_count.columns = ['country', 'country_count']
            
            last4_df = last4_df.reset_index().merge(last4_group_count, how="left").set_index('index')
            last4_df = last4_df.reset_index().merge(last4_country_count, how="left").set_index('index')
...
print(str(invalid_draws_enhanced_n/100) + " of draws were invalid using draw_clubs_country")
...
print(str(invalid_draws_enhanced_alt_n/100) + " of draws were invalid using draw_clubs_country_alt")
...
def draw_clubs_order(last16_df):
    ...
    for r in runners:
        unchosen_clubs.loc[unchosen_clubs['club'] == r, 'eli_clubs'] = eligible_clubs(r, unchosen_clubs).shape[0]
...
print(str(invalid_draws_order_n/100) + " of draws were invalid using draw_clubs_order")
